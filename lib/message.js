...
function buildFormatter(eventName, opts) {
  ...
  function formatter(suite) {
    ...
    const sessionId = this.baseReporter.stats.runners[suite.cid].sessionID;

    const out = pattern.replace(/\[([a-z]+)\]/gi, function (m, g) {
      return formatProp(g, suite);
    });

    return out.replace("flowId=''", `flowId='${sessionId}'`);
  }
}
